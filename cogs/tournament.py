...
class Tournament(commands.Cog):
    ...
    def make_tournament_embed(self, ctx):
        desc = "Information about Tournament related commands! **[use lockout tournament <command>]**\n\n"
        ...
        embed.set_footer(
            text="Use the prefix lockout before each command. For detailed usage about a particular command, type lockout help <command>")
    ...
    @commands.group(brief='Commands related to tournaments! Type lockout tournament for more details', invoke_without_command=True, aliases=['tourney'])
    ...
    async def faq(self, ctx):
        data = [["What formats are supported?",
                ...
                ["How to setup a tournament?",
                 "To setup a tournament type `lockout tournament setup x y` where x is an integer in the range 0 to 2 (Single Elim, Double Elim, Swiss) denoting tournament type and y is the name of the tournament."],
                ["How to register/unregister for the tournament?",
                 "To register type `lockout tournament register` and to unregister type `lockout tournament unregister`. Admins can forcefully unregister a user by typing `lockout tournament _unregister <handle>`."],
                ["How to start the tournament?",
                 "To start the tournament, type `lockout tournament begin`"],
                ["How to compete once the tournament has started?",
                 "To compete, use the `lockout round` command and challenge your opponent. The bot will automatically ask you if you want the result of the round to be counted in the tournament and update the tournament bracket once the round is complete."],
                ["What if my opponent doesn't show up or leaves the server without completing the matches?",
                 "You can ask an admin to use the command `lockout tournament forcewin <handle>` where handle is the winners codeforces handle."],
                ["What if the bot accidentally gives victory to the wrong user?",
                 "You can ask an admin to invalidate the match results by typing `lockout tournament match_invalidate x` where x is match number (can be accessed from challonge page of the tournament). This will also reset the subsequent matches whose result depends on this match"]]
    ...
    async def setup(self, ctx, tournament_type: int, *, tournament_name: str):
        ...
        **tournament_type:** Integer (0: single elimination, 1: double elimination, 2: swiss)
        ...
        desc = f"""
               ...
               To register, type `lockout tournament register` (Max registrations: **{MAX_REGISTRANTS}**)
               To unregister, type `lockout tournament unregister`
               To start the tournament, type `lockout tournament begin` 
               """
    ...
    async def begin(self, ctx):
        ...
        if tournament_info.status == 2:
            await discord_.send_message(ctx, f"The tournament has already begun! Type `lockout tournament matches` or `lockout tournament info` to view details about the tournament")
            return
        ...
        if tournament_info.status == 0:
            ...
            desc += f"Note that the tournament has **not** started yet. Once everyone has made their predictions, type `lockout tournament begin` for the tournament to officially begin\n\n"
            ...
            desc += f"`lockout tournament matches`: View a list of future matches of the tournament\n"
            desc += f"`lockout tournament info`: View general info about the tournament\n"
            desc += f"`lockout tournament forcewin <handle>`: Grant victory to a user without conducting the match\n"
            desc += f"`lockout tournament invalidate`: Invalidate the tournament\n"
    ...
    async def matches(self, ctx):
        ...
        if not tournament_info:
            ...
            return
        if tournament_info.status != 2:
            await discord_.send_message(ctx, "The tournament has not begun yet, type `lockout tournament begin` to start the tournament")
            return
    ...
    async def match_invalidate(self, ctx, idx: int):
        ...
        if tournament_info.status != 2:
            await discord_.send_message(ctx,
                                        "The tournament has not begun yet, type `lockout tournament begin` to start the tournament")
            return
