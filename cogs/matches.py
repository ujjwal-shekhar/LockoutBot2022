...
class Matches(commands.Cog):
    ...
    def make_match_embed(self, ctx):
        desc = "Information about Matches related commands! **[use lockout match <command>]**\n\n"
    ...
    @commands.group(brief='Commands related to matches. Type lockout match for more details', invoke_without_command=True)
    ...
    async def challenge(self, ctx, member:discord.Member, rating: int):
        ...
        await ctx.send(f"{ctx.author.mention} has challenged {member.mention} to a match with problem ratings from "
                       f"{rating} to {rating+400} and lasting {duration} minutes. Type `lockout match accept` within 60 seconds to accept")
        tme = int(time.time())
    ...
    async def accept(self, ctx):
        ...
        embed.set_footer(text=f"You can now conduct tournaments using the bot.\n"
                              f"Type lockout tournament faq for more info")
