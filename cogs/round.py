...
RATING_LIST_DIV_2 = [1200, 1200, 1400, 1600, 1800]
...
class Round(commands.Cog):
    ...
    def make_round_embed(self, ctx):
        desc = "Information about Matches related commands! **[use lockout round <command>]**\n\n"
        ...
        embed.set_footer(
            text="Use the prefix lockout before each command. For detailed usage about a particular command, type lockout help match <command>")
    ...
    @commands.group(brief='Commands related to rounds! Type lockout round for more details', invoke_without_command=True)
    ...
    async def challenge(self, ctx, *users: discord.Member):
        ...
        if len(users) == 0:
            await discord_.send_message(ctx, f"The correct usage is `lockout round challenge @user1 @user2...`")
            return
        ...
        for i in users:
            if not self.db.get_handle(ctx.guild.id, i.id):
                await discord_.send_message(ctx, f"Handle for {i.mention} not set! Use `lockout handle identify` to register")
                return
...
#
                ...
                if resp[1]:
                    ...
                    if round_info.tournament == 1:
                        ...
                        if ranklist[1].rank == 1 and tournament_info.type != 2:
                            ...
                            if not res[0]:
                                await discord_.send_message(channel, res[1] + "\n\nIf you think this is a mistake, type `lockout tournament forcewin <handle>` to grant victory to a user")
                            else:
    ...
    async def custom(self, ctx, *users: discord.Member):
        ...
        users = list(set(users))
        if len(users) == 0:
            await discord_.send_message(ctx, f"The correct usage is `lockout round custom @user1 @user2...`")
            return
        if ctx.author not in users:
            users.append(ctx.author)
        if len(users) > MAX_ROUND_USERS:
